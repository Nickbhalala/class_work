(module
    ...
    block $done
      loop $repeat
        ...
        (call $draw_artwork
              ...
              (local.get $y) ;; y coordinate to draw to in the HTML canvas
        )        
 ...
 (func $step (export "step") (param $progress i32)
    ...
    block $done
      loop $repeat
        (br_if $done (i32.ge_u (local.get $count) (i32.const 22)))
        (call $draw_row_of_cherries (i32.mul (local.get $count) (i32.const 16)))
         (local.set $count (i32.add (i32.const 1) (local.get $count)))
        br $repeat
      end $repeat
    end $done
